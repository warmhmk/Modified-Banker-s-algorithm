...
def process1():
    ...
    # simulate process running for 1 seconds
    time.sleep(1)
...
def updateWork(i):
    ...
    if safe[i] == True:
        next = [allocation[i][k] for k in range(len(allocation[i]))]
...
def main():
    ...
    # allocation (global to share amoung the processes)
    global allocation
    #                   A B C
    allocation =      [[4,1,2], # p0
                       [4,2,4]] # p1
